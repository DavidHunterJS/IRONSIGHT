...
- [ ] **Walla's timestamps are three hours off, and after #38 that is what decides who fills the rest of iran-israel.** Walla labels Israel local time as GMT. The recency sort measures distance from now in either direction, so a Walla item more than about three hours old reads three hours younger than it is and wins the fill against every honestly dated outlet; only its very newest items fold back and read older. With every source now guaranteed its three newest rows, the remaining 37 go by recency, and Walla still holds 15 of 15 in iran-israel while Haaretz holds 8 and Times of Israel 8. JPost shows the same skew. See the Feed health entry below for the measurement.
...
- [ ] **Walla and JPost timestamps run up to ~3 hours in the future.** Measured 2026-09-10: 11 iran-israel rows dated 0.1-2.6 hours ahead, all Walla or JPost, all labelled `GMT`. The size fits Israel local time (UTC+3 in summer) written with the wrong zone. The recency sort uses distance from now in either direction, so an item under about three hours old ranks as if it were older than it is, and anything older ranks about three hours younger - which is most of them, and why Walla wins the recency fill (see Next up). `timeAgo` prints the absolute value. Not an age problem, so the age window leaves future dates alone.
