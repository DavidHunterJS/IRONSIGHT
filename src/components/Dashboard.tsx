...
import { AttributionNote, DisclaimerBar } from '@/components/Disclaimer';
...
export default function Dashboard() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <header className="border-b border-[var(--border-color)] bg-[var(--bg-secondary)] shrink-0">
        <div className="flex items-center justify-between px-4 py-1.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold tracking-[3px] text-[var(--cyan)]">{BRAND.name}</h1>
      ...
      {/* Bottom status bar */}
      <footer className="border-t border-[var(--border-color)] bg-[var(--bg-secondary)] px-4 py-1 flex items-center justify-between gap-4 text-[9px] text-[var(--text-secondary)] shrink-0 flex-wrap">
        <span className="truncate">
          FEEDS: NEWS | GDELT | TELEGRAM | OPENSKY | OCHA | YAHOO FIN |{' '}
          {config.client.alertSystemName.toUpperCase()} | NASA FIRMS | ADSB.LOL
        </span>
        <div className="flex items-center gap-4 flex-wrap">
          {/* Disclaimer and upstream MIT credit are permanent fixtures, not
              dismissible chrome — see src/components/Disclaimer.tsx. */}
          <DisclaimerBar />
          <AttributionNote />
        </div>
      </footer>
